...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	public synchronized int search(final String text, boolean ignoreCase, boolean wholeText, boolean backward,
		...
		if (!myCurrentPage.StartCursor.isNull()) {
			...
			if (count > 0) {
				...
				gotoMark(wholeText ? (backward ? myModel.getLastMark() : myModel.getFirstMark())
	...
	public synchronized void onScrollingFinished(PageIndex pageIndex) {
		TopView.changePage(pageIndex.toString());
		if (fbreader == null) {
			fbreader = new FBReader();
		}
		fbreader.navigate(false);
		switch (pageIndex) {
		case current:
			break;
		case previous: {
	...
	private int sizeOfTextBeforeCursor(ZLTextWordCursor wordCursor) {
		...
		if (paragraphLength > 0) {
			sizeOfText += (myModel.getTextLength(paragraphIndex) - sizeOfText) * wordCursor.getElementIndex()
					/ paragraphLength;
		}
		return sizeOfText;
	}
	...
	private synchronized float computeCharsPerPage() {
		...
		final int effectiveHeight = (int) (textHeight
				- (getTextStyle().getSpaceBefore(metrics()) + getTextStyle().getSpaceAfter(metrics()) / 2)
						/ charsPerParagraph);
	...
	private void drawTextLine(ZLTextPage page, List<ZLTextHighlighting> hilites, ZLTextLineInfo info, int from,
			int to) {
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex
			...
			if (element == area.Element) {
				...
				if (element instanceof ZLTextWord) {
					...
					context.drawImage(areaX, areaY, imageElement.ImageData, getTextAreaSize(),
							getScalingType(imageElement), getAdjustingModeForImages());
		...
		if (index != to) {
			...
			final ZLTextHighlighting hl = getWordHilite(pos, hilites);
			final ZLColor hlColor = hl != null ? hl.getForegroundColor() : null;
			drawWord(area.XStart, area.YEnd - context.getDescent() - getTextStyle().getVerticalAlign(metrics()), word,
					start, len, area.AddHyphenationSign,
					hlColor != null ? hlColor : getTextColor(getTextStyle().Hyperlink));
		}
	}
	...
	private ZLTextLineInfo processTextLine(ZLTextPage page, ZLTextParagraphCursor paragraphCursor, final int startIndex,
			final int startCharIndex, final int endIndex, ZLTextLineInfo previousInfo) {
		final ZLTextLineInfo info = processTextLineInternal(page, paragraphCursor, startIndex, startCharIndex, endIndex,
				previousInfo);
	...
	private ZLTextLineInfo processTextLineInternal(ZLTextPage page, ZLTextParagraphCursor paragraphCursor,
			final int startIndex, final int startCharIndex, final int endIndex, ZLTextLineInfo previousInfo) {
		final ZLPaintContext context = getContext();
		...
		do {
			...
			if (!allowBreak) {
				element = paragraphCursor.getElement(currentElementIndex);
				allowBreak = previousElement != ZLTextElement.NBSpace && element != ZLTextElement.NBSpace
			...
			if (element instanceof ZLTextWord) {
				...
				if ((word.Length > 3 && spaceLeft > 2 * context.getSpaceWidth())
					...
					for (int right = word.Length - 1, left = currentCharIndex; right > left;) {
						...
						if (m1 > left) {
							final int w = getWordWidth(word, currentCharIndex, m1 - currentCharIndex,
					...
					if (hyphenationPosition == currentCharIndex && info.EndElementIndex == startIndex) {
						...
						while (right > left) {
							final int mid = (right + left + 1) / 2;
							final int w = getWordWidth(word, currentCharIndex, mid - currentCharIndex,
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex; ++wordIndex, charIndex = 0) {
			...
			if (element == ZLTextElement.HSpace) {
				if (wordOccurred && spaceCounter > 0) {
					...
					|| element instanceof ZLTextVideoElement || element instanceof ExtensionElement) {
				...
				page.TextElementMap
						.add(new ZLTextElementArea(paragraphIndex, wordIndex, charIndex, length - charIndex, true, // is
																													// last
																													// in
																													// element
								false, // add hyphenation sign
		...
		if (!endOfParagraph) {
			...
			if (len > 0) {
				...
				page.TextElementMap.add(new ZLTextElementArea(paragraphIndex, wordIndex, 0, len, false, // is
																										...
																										// in
																										// element
						addHyphenationSign, changeStyle, getTextStyle(), word, x, x + width - 1, y - height + 1,
						y + descent, columnIndex));
			}
		}
	}
	...
	private synchronized void preparePaintInfo(ZLTextPage page) {
		...
		case PaintStateEnum.TO_SCROLL_FORWARD:
			if (!page.EndCursor.isEndOfText()) {
				...
				if (!startCursor.isNull()) {
					...
					if (!page.isEmptyPage() && (myScrollingMode != ScrollingMode.KEEP_LINES
		...
		case PaintStateEnum.TO_SCROLL_BACKWARD:
			if (!page.StartCursor.isStartOfText()) {
				...
				case ScrollingMode.SCROLL_LINES:
					page.StartCursor
							.setCursor(findStart(page, page.StartCursor, SizeUnit.LINE_UNIT, myOverlappingValue));
					break;
				case ScrollingMode.SCROLL_PERCENTAGE:
					page.StartCursor.setCursor(findStart(page, page.StartCursor, SizeUnit.PIXEL_UNIT,
	...
	private ParagraphSize paragraphSize(ZLTextPage page, ZLTextWordCursor cursor, boolean beforeCurrentPosition,
			int unit) {
		...
		final int endElementIndex = beforeCurrentPosition ? cursor.getElementIndex()
				: paragraphCursor.getParagraphLength();
	...
	private void skip(ZLTextPage page, ZLTextWordCursor cursor, int unit, int size) {
		...
		while (!cursor.isEndOfParagraph() && size > 0) {
			info = processTextLine(page, paragraphCursor, cursor.getElementIndex(), cursor.getCharIndex(),
					endElementIndex, info);
	...
	/*
	 * public void resetRegionPointer() { myOutlinedRegionSoul = null;
	 * myShowOutline = true; }
	 */
